/*
 Ce fichier genenre un jeu de test.

 Voici les données générées :
		Sites           : 2 (Cergy, Pau)
...
SET DEFINE OFF
-- DEFINE OFF :	evite que SQL*Plus interprete les '&' dans les chaines comme des variables de substitution. (ex : 'Dept Biotech & Chimie').
ALTER SESSION SET NLS_DATE_FORMAT = 'YYYY-MM-DD';

/*
	Package (header) du jeu de test.
*/
...
CREATE OR REPLACE PACKAGE BODY pkg_jeu_test AS
	...
	--	 retourne un num de série aléatoire (string)
	...
	-- retourne une date aléatoire passée
	-- param : nbre de jours max à remonter par rapport a la date actuelle
	...
	*/
	...
	/*
		Peuplement des sites (Cy Tech + Pau)
	*/
	PROCEDURE peupler_sites IS
	BEGIN
		INSERT INTO sites(id, nom, adresse, ville, code_postal, telephone)
		VALUES (seq_sites.NEXTVAL, 'CY Tech Cergy', 'Avenue du Parc', 'Cergy', '95000', '0134256900');

		INSERT INTO sites(id, nom, adresse, ville, code_postal, telephone)
		VALUES (seq_sites.NEXTVAL, 'CY Tech Pau', '2 boulevard Lucien Favre', 'Pau', '64075', '0559059090');

		DBMS_OUTPUT.PUT_LINE('  Sites : 2 (Cergy, Pau)');
	END peupler_sites;

	/*
		Peuplement de hierarchy_level.
	*/
	...
	BEGIN
		...
		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Cergy', v_id_racine, c_site_cergy, 1, 'CY Tech > Cergy')
		RETURNING id INTO v_id_cergy;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Pau', v_id_racine, c_site_pau, 1, 'CY Tech > Pau')
		RETURNING id INTO v_id_pau;

		-- Niveau 2 - Cergy : services + 4 departements pedagogiques
		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Gestion', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Gestion')
		RETURNING id INTO v_ent_cergy_gestion;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'IT', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > IT')
		RETURNING id INTO v_ent_cergy_it;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Administration', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Administration')
		RETURNING id INTO v_ent_cergy_admin;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Bureau des profs', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Bureau des profs')
		RETURNING id INTO v_ent_cergy_profs;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Dept Informatique', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Dept Informatique')
		RETURNING id INTO v_ent_cergy_info;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Dept Maths appliquees', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Dept Maths appliquees')
		RETURNING id INTO v_ent_cergy_maths;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Dept Biotech & Chimie', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Dept Biotech & Chimie')
		RETURNING id INTO v_ent_cergy_biotech;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Dept Genie Civil', v_id_cergy, c_site_cergy, 2, 'CY Tech > Cergy > Dept Genie Civil')
		RETURNING id INTO v_ent_cergy_gc;

		-- Niveau 2 - Pau : services + 2 departements pedagogiques
		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Gestion', v_id_pau, c_site_pau, 2, 'CY Tech > Pau > Gestion')
		RETURNING id INTO v_ent_pau_gestion;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'IT', v_id_pau, c_site_pau, 2, 'CY Tech > Pau > IT')
		RETURNING id INTO v_ent_pau_it;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Administration', v_id_pau, c_site_pau, 2, 'CY Tech > Pau > Administration')
		RETURNING id INTO v_ent_pau_admin;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Bureau des profs', v_id_pau, c_site_pau, 2, 'CY Tech > Pau > Bureau des profs')
		RETURNING id INTO v_ent_pau_profs;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Dept Informatique', v_id_pau, c_site_pau, 2, 'CY Tech > Pau > Dept Informatique')
		RETURNING id INTO v_ent_pau_info;

		INSERT INTO hierarchy_level(id, nom, hierarchy_level_parent_id, site_id, niveau, nom_complet)
		VALUES (seq_hierarchy_level.NEXTVAL, 'Dept Maths appliquees', v_id_pau, c_site_pau, 2, 'CY Tech > Pau > Dept Maths appliquees')
	...
	END peupler_hierarchy_level;
  	...
  	/*
		Initialisation des localisations : 
			-> batiments :	Condorcet/Cauchy/Turing a Cergy/Parc, Fermat a Cergy/Fermat, PAU a Pau
	...
	*/
	...
	BEGIN
		...
		VALUES (seq_localisations.NEXTVAL, p_nom_bat,
				'Batiment ' || p_nom_bat, p_hierarchy_level_id, NULL, p_nom_bat, NULL, NULL)
		RETURNING id INTO v_id_bat;

		-- Salles : pour chaque etage, on a p_nb_salles_par_etage salles
		FOR et IN 1..p_nb_etages LOOP
			FOR sa IN 1..p_nb_salles_par_etage LOOP
				INSERT INTO localisations(id, nom, nom_complet, hierarchy_level_id, localisation_parent_id, batiment, salle, etage)
	...
	END peupler_localisations;
  	...
  	/*
		Peuplement de la table fabricants.
	*/
	...
	BEGIN
		FOR i IN 1..20 LOOP
			...
			INSERT INTO fabricants(id, nom)
			VALUES (seq_fabricants.NEXTVAL, v_nom);
		END LOOP;
		DBMS_OUTPUT.PUT_LINE('  Fabricants : ' || TO_CHAR(20));
	END peupler_fabricants;

	/*
		Peuplement de la table etats.
	*/
	...
	BEGIN
		FOR i IN 1..8 LOOP
			v_nom := v_etats_lib(i);
			INSERT INTO etats(id, nom, etat)
			VALUES (seq_etats.NEXTVAL, v_nom, v_nom);
		END LOOP;
		DBMS_OUTPUT.PUT_LINE('  Etats : ' || TO_CHAR(8));
	END peupler_etats;

	/*
		Peuplement de la table types_ordinateur.
	*/
	...
	/*
		Peuplement de la table modeles_ordinateur.
	*/
	...
	/*
		Peuplement de la table profils avec 5 profils.
	*/
	...
	*/
	...
	/*
		Procedure d'insertion d'un utilisateur.
	*/
	...
	/*
		Procedure de peuplement des pcs fixes à Cauchy avec :
			-> 140 PC fixes à l'etage 2 (4 salles x 35 PC), sans utilisateur
			-> 1 PC portable par utilisateur, dans sa localisation
	*/
	...
	/*
		Procédure de peuplement de Pcs portables.
	*/
	...
	/*
		Procédure de peuplement de speriphériques : souris+clavier des PC fixes + videoprojs + imprimantes
	*/
	...
	/*
		Procédure de peuplement des telephones fices : 
			1 par utilisateur du service Administration
	*/
  ...
  /*
	Procédure de peuplement des logiciels.
  */
  ...
  END peupler_installations;
	...
	/*
		Procédure de peuplement du réseau, cad :
			1 switch + 1 routeur WiFi par salle + 1 borne WiFi par etage
	...
	*/
	...
	/*
		Procédure jouant le role de chef d'orchestre
	*/
	PROCEDURE generer_tout(
		p_nb_etudiants  NUMBER DEFAULT 2500,
		p_nb_profs      NUMBER DEFAULT 80,
	...
	/*
		Procédure de réinitialisation des données.
	*/
...
/*
	Pour generer le jeu de test par defaut (structure CY Tech reelle) : EXEC pkg_jeu_test.generer_tout;
	Pour augmenter les volumes (tests de perf) : EXEC pkg_jeu_test.generer_tout(p_nb_etudiants => 10000, p_nb_profs => 200);
	Pour repartir de zero : EXEC pkg_jeu_test.reset_donnees;
*/
